    ...
    render() {
        return (
            <div>
                ...
                >
                    {(state) => (
                    <div className="squash-and-stretch squash-and-stretch--css-transition" style={{
                        ...defaultStyle,
                ...
                <div onClick={this._handleClick.bind(this)} style={{fontSize: '5rem'}}>
...
export class ReactTransitionGroupExample extends React.Component {
    ...
    componentDidMount() {
        this._interval = setInterval(this._handleClick.bind(this), 1000);
    }
    ...
    <div>
        {
            Object.keys(springs).map((key) => (
                <Motion defaultStyle={{x: 0}} style={{x: spring(500, springs[key])}} key={key}>
                    {interpolatingStyle => <div className="squash-and-stretch squash-and-stretch--physics" style={{transform: `translateX(${interpolatingStyle.x}px)`}} />}
                </Motion>
            ))
        }
   </div>
